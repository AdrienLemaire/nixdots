...
# Determine theme based on GNOME preference
THEME=$(gsettings get org.gnome.desktop.interface color-scheme | sed "s/'//g;s/prefer-//")

# Set the theme in all active Neovim sessions
for SOCKET in /run/user/1000/nvim.*; do
	if [ -S "$SOCKET" ]; then
		nvim --server "$SOCKET" --remote-send ":set bg=$THEME<CR>"
	fi
done
